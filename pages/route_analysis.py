...
with tab_segments:
    ...
    if not MODELS_AVAILABLE:
        st.warning("Prediction models are not available")
        st.info("""
        **Why models aren't loading:**
        - Model files may be too large for Streamlit Cloud free tier
        - HuggingFace rate limits may be in effect
        
        """)
    elif sel_ref not in route_models:
        st.info(f"No trained model available for route {sel_ref}")
        st.write(f"Available routes with models: {len(route_models)}")
    else:
        # Try to build segment predictions
        seg_df, seg_err = build_segment_times_from_model(
            sel_ref, routes, route_models, feature_columns
        )
        
        if seg_err or (seg_df is None):
            msg = seg_err if isinstance(seg_err, str) else "Prediction unavailable for this route."
            st.info(msg)
        else:
            ...
            with k1: 
                st.markdown(
                    f'<div class="card"><div class="dim">Total Time</div>'
                    f'<div class="big">{total_min:.1f} min</div></div>', 
                    unsafe_allow_html=True
                )
            with k2: 
                st.markdown(
                    f'<div class="card"><div class="dim">Distance</div>'
                    f'<div class="big">{total_km:.2f} km</div></div>', 
                    unsafe_allow_html=True
                )
            with k3: 
                st.markdown(
                    f'<div class="card"><div class="dim">Avg Pred Speed</div>'
                    f'<div class="big">{avg_spd:.1f} km/h</div></div>', 
                    unsafe_allow_html=True
                )
            ...
            figC = px.line(
                ...
                labels={
                    ...
                    "cumulative_min": "Cumulative Time (min)"
                }
            )
